...
export const campaign_messages = pgTable(
  "campaign_messages",
  {
  ...
  (table) => [
    ...
    pgPolicy("[ANON] - INSERT", {
      as: "permissive",
      for: "insert",
      to: ["anon"],
      withCheck: sql`true`,
    }),
    pgPolicy("[AUTH] - DELETE", {
      as: "permissive",
      for: "delete",
      to: ["authenticated"],
    }),
    pgPolicy("[ANON] - DELETE", {
      as: "permissive",
      for: "delete",
      to: ["anon"],
    }),
    pgPolicy("[ANON] - SELECT", {
      as: "permissive",
      for: "select",
      to: ["anon"],
    }),
    pgPolicy("[ANON] - UPDATE", {
      as: "permissive",
      for: "update",
      to: ["anon"],
    }),
    pgPolicy("[AUTH] - SELECT", {
      as: "permissive",
      for: "select",
      to: ["authenticated"],
    }),
    pgPolicy("[AUTH] - INSERT", {
      as: "permissive",
      for: "insert",
      to: ["authenticated"],
    }),
    pgPolicy("[AUTH] - UPDATE", {
      as: "permissive",
      for: "update",
      to: ["authenticated"],
    }),
...
export const whatsapp = pgTable(
  ...
  (table) => [
    index("idx_whatsapp_user_id").using(
      "btree",
      table.company_id.asc().nullsLast().op("uuid_ops")
    ),
    foreignKey({
      columns: [table.campaign_id],
      foreignColumns: [campaigns.id],
...
export const whatsapp_tracking = pgTable(
  "whatsapp_tracking",
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
    created_at: timestamp({ withTimezone: true, mode: "string" })
  ...
  (table) => [
    index("idx_whatsapp_tracking_user_id").using(
      "btree",
